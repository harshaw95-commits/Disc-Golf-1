...
const state = {
  ...
  liveTools: JSON.parse(JSON.stringify(LIVE_TOOL_PRESETS)),
};

const elements = {
  cameraFeed: document.getElementById('cameraFeed'),
  cameraShell: document.getElementById('cameraShell'),
  liveMeasureOverlay: document.getElementById('liveMeasureOverlay'),
  cameraStatus: document.getElementById('cameraStatus'),
  topGuide: document.getElementById('topGuide'),
  sideGuide: document.getElementById('sideGuide'),
  ...
  topToolMetrics: document.getElementById('topToolMetrics'),
  sideToolMetrics: document.getElementById('sideToolMetrics'),
  liveTopToolBtn: document.getElementById('liveTopToolBtn'),
  liveSideToolBtn: document.getElementById('liveSideToolBtn'),
  topToolCard: document.getElementById('topToolCard'),
  sideToolCard: document.getElementById('sideToolCard'),
...
function renderLiveToolMetrics(view) {
  const target = view === 'top' ? elements.topToolMetrics : elements.sideToolMetrics;
  const measurements = getToolSegmentMetrics(view);
  target.innerHTML = measurements
    .map(
      (segment) => `
        <div class="tool-metric tool-${segment.colorClass}">
          <strong>${segment.label}</strong>
          <span>${segment.pixelText}</span>
          <small>${segment.estimatedText}</small>
          <small>${segment.secondaryLabel}</small>
        </div>
      `,
    )
    .join('');
...
function renderAllLiveToolMetrics() {
  renderLiveToolMetrics('top');
  renderLiveToolMetrics('side');
}

function setActiveLiveTool(view) {
  state.activeLiveTool = view;
  elements.liveTopToolBtn.classList.toggle('active', view === 'top');
  elements.liveSideToolBtn.classList.toggle('active', view === 'side');
  elements.topToolCard.classList.toggle('active', view === 'top');
  elements.sideToolCard.classList.toggle('active', view === 'side');
  elements.topGuide.classList.toggle('active', view === 'top');
  elements.sideGuide.classList.toggle('active', view === 'side');
...
function renderLiveMeasureOverlay() {
  ...
  overlay.innerHTML = '';
  state.liveTools[state.activeLiveTool].forEach((segment) => {
    const line = document.createElement('div');
    line.className = `measure-line ${segment.colorClass}`;
    positionSegmentElement(line, segment.start, segment.end);

    const label = document.createElement('div');
    label.className = `measure-label ${segment.colorClass}`;
    label.textContent = segment.label;
    label.style.left = `${((segment.start.x + segment.end.x) / 2) * 100}%`;
    label.style.top = `${((segment.start.y + segment.end.y) / 2) * 100}%`;
    ...
    ['start', 'end'].forEach((handleKey) => {
      ...
      handle.type = 'button';
      handle.className = `measure-handle ${segment.colorClass}`;
      handle.dataset.segmentKey = segment.key;
      handle.dataset.handleKey = handleKey;
      const point = segment[handleKey];
      handle.style.left = `${point.x * 100}%`;
      handle.style.top = `${point.y * 100}%`;
      handle.setAttribute('aria-label', `${segment.label} ${handleKey} handle`);
...
function beginHandleDrag(event) {
  ...
  state.draggingHandle = {
    segmentKey: handle.dataset.segmentKey,
    handleKey: handle.dataset.handleKey,
  };
...
function attachEvents() {
  ...
  elements.liveTopToolBtn.addEventListener('click', () => setActiveLiveTool('top'));
  elements.liveSideToolBtn.addEventListener('click', () => setActiveLiveTool('side'));
  elements.liveMeasureOverlay.addEventListener('pointerdown', beginHandleDrag);
  window.addEventListener('pointermove', updateDraggedHandle);
  window.addEventListener('pointerup', endHandleDrag);
  window.addEventListener('resize', () => {
    renderLiveMeasureOverlay();
    renderAllLiveToolMetrics();
  });
  elements.topReferenceWidth.addEventListener('input', renderAllLiveToolMetrics);
  elements.sideReferenceWidth.addEventListener('input', renderAllLiveToolMetrics);
}
